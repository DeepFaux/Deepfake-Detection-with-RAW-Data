...
def init_dfdc():     
    label=pd.read_csv('data/DFDC/test/labels.csv',delimiter=',')
    folder_list=[f'data/DFDC/test/{i}' for i in label['filename'].tolist()] 
...
def init_dfdcp(phase='test'):
    ...
    all_img_list=[]
    all_label_list=[]
    path = 'data/DFDC/dfdc_preview_set'
    with open(f'{path}/dataset.json') as f:
        df=json.load(f)
    fol_lab_list_all=[[f"{path}/{k.split('/')[0]}/{k.split('/')[-1]}",df[k]['label']=='fake'] for k in df if df[k]['set']==phase_integrated[phase]]
    ...
    folder_list=glob(f'{path}/method_*/*/*/*.mp4')+glob('f{path}/original_videos/*/*.mp4')
