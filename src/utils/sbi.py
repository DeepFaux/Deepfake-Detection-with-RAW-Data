    ...
    def __getitem__(self,idx):
        ...
        while flag:
            try:		
                ...
                # resized_image = cv2.resize(temp_mask, (224, 224))
                # # print(np.max(resized_image))
                # cv2.imwrite(f"mask/mask_{idx}.png", resized_image)
                ...
                img_f=cv2.resize(img_f,self.image_size,interpolation=cv2.INTER_LINEAR).astype('float32')/255 #original image 
                img_r=cv2.resize(img_r,self.image_size,interpolation=cv2.INTER_LINEAR).astype('float32')/255 #fake image
                mask_f_temp=cv2.resize(mask_f,self.image_size,interpolation=cv2.INTER_LINEAR).astype('float32') #sahar mask

                img_f=img_f.transpose((2,0,1))
                img_r=img_r.transpose((2,0,1))
                # mask_f_temp=mask_f_temp.transpose((2,0,1))
                flag=False
            except Exception as e:
                print(e)
                idx=torch.randint(low=0,high=len(self),size=(1,)).item()
        return img_f,img_r, mask_f_temp 
